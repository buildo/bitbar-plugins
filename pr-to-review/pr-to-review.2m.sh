...
PR_COUNT=`echo $GITHUB_COM_RESULT | jq '.total_count'`

function colorForCount {
  if [ $PR_COUNT = 0 ]; then echo 'green'; else echo 'red'; fi
}
...
echo " $PR_COUNT | color=$COLOR image=$MERGE_ICON href=https://github.com/pulls/review-requested color=$GITHUB_COM_COLOR"
